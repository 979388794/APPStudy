...
public class CommunicationLocal extends CommunicationDefinitionIpm<ResSerializableBean<ArrayList<DiagnosisInfoList>>> {

    private int id;

    public CommunicationLocal(int id) {
        this.id = id;
    }

    @Override
    public ResSerializableBean<ResSerializableBean<ArrayList<DiagnosisInfoList>>> getData(String data) {
        ResSerializableBean<ResSerializableBean<ArrayList<DiagnosisInfoList>>> resSerializableBean = super.getData(data);
