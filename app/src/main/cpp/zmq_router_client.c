...
int zmq_route_client_init(const char *client_name, const char *server_name, const char *end_point) {
    ...
    do {
        ...
        rc = zmq_setsockopt(p_zmq_router->router, ZMQ_ROUTING_ID, p_zmq_router->client_name,
                            strlen(p_zmq_router->client_name) + 1);
        if (rc < 0) {
...
int zmq_route_client_send_sync(const char *send_msg, int send_msg_len, char *recv_msg,
                               int *recv_msg_len) {
    int rc = 0, ret = 0;
    char buf[1] = {0};
    int recv_len = 0;
    ...
    sleep(1);
    rc = zmq_send(p_zmq_router->router, p_zmq_router->server_name,
                  strlen(p_zmq_router->server_name) + 1, ZMQ_DONTWAIT | ZMQ_SNDMORE);
    ...
    rc = zmq_recv(p_zmq_router->router, p_zmq_router->server_name,
                  sizeof(p_zmq_router->server_name), 0);
    LOGD("recv addr:%d,%s\n", rc, rc > 0 ? p_zmq_router->server_name : "null");
    if (rc > 0) {
        rc = zmq_recv(p_zmq_router->router, buf, sizeof(buf), ZMQ_DONTWAIT);
        LOGD("recv data type:%d,%d\n", rc, buf[0]);
        try_again:
        rc = zmq_recv(p_zmq_router->router, recv_msg + recv_len, *recv_msg_len - recv_len,
                      ZMQ_DONTWAIT);
